    ...
    <div className={`fixed inset-0 z-[9999] flex items-center justify-center overflow-hidden bg-black transition-all duration-500 ${phase === 'open' ? 'pointer-events-none bg-transparent' : 'pointer-events-auto'}`}>
      ...
      <div className={`absolute inset-0 z-30 flex items-center justify-center transition-transform duration-100 ${phase === 'open' ? 'animate-shake' : ''}`}>
        {[0, 60, 120, 180, 240, 300].map((deg, i) => (
          <div
            key={i}
            // Mobile: Wider (200vw) and Taller (60vh) to ensure coverage on portrait screens
            // Desktop: Original sizing (100vw / 50vh)
      ...
      <div className={`relative z-40 transition-all duration-500 ${phase === 'open' ? 'scale-[5] opacity-0 rotate-180' : 'scale-100 opacity-100'}`}>
        ...
        {/* Mobile: w-32 h-32 | Desktop: w-40 h-40 */}
        <div className="w-32 h-32 md:w-40 md:h-40 bg-[#111] border border-zinc-700 clip-path-hexagon flex items-center justify-center relative overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.8)]">
            <svg width="80" height="80" viewBox="0 0 100 100" className={`transition-all duration-300 scale-75 md:scale-100 ${phase === 'unlocking' ? 'text-white drop-shadow-[0_0_15px_rgba(255,255,255,1)]' : 'text-orange-600'}`}>
      ...
      {/* LAYER 4: LOGS */}
      {/* Mobile: bottom-8 left-4 | Desktop: bottom-10 left-10 */}
